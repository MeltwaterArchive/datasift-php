    ...
    protected function __construct($user, $definition, $eventHandler)
    {
        if (! ($user instanceof DataSift_User)) {
            throw new DataSift_Exception_InvalidData(
                'Please supply a valid DataSift_User object when creating a DataSift_StreamConsumer object.'
            );
        }
        ...
        if (is_array($definition) && count($definition) > 0) {
            ...
            throw new DataSift_Exception_InvalidData(
                'The definition must be a CSDL string, a DataSift_Definition object, or an array of stream hashes.'
            );
        }
        ...
        if (! ($eventHandler instanceof DataSift_IStreamConsumerEventHandler)) {
            throw new DataSift_Exception_InvalidData(
                'Your event handler object must implement the DataSift_IStreamConsumerEventHandler interface.'
            );
        }
