...
class DataSift_StreamConsumer_HTTP extends DataSift_StreamConsumer
{
    ...
    protected function onStart()
    {
        do {
            ...
            // requested.
            while ($this->_conn && ! feof($this->_conn) &&
                ($count = stream_select($read, $write, $except, $this->_read_timeout))) {
                ...
                while ($this->_state == parent::STATE_RUNNING &&
                    ($line = fgets($this->_conn, $this->_max_line_length)) !== false) {
                    if (strlen($line) == 0) {
                        // End of the chunk
                        break;
                    }
            ...
            $this->onDisconnect();
        } while ($this->_conn && (! feof($this->_conn) || $this->_auto_reconnect) &&
    ...
    private function connect()
    {
        ...
        if ($this->_is_multi) {
            $url = $protocol . '://' . $this->_user->getStreamUrl() . 'multi';
        } else {
            $url = $protocol . '://' . $this->_user->getStreamUrl() . $this->_definition->getHash();
        }
        $url = parse_url($url);
